...
// overloaded stream operator
   ...
   friend std::istream& operator>>( std::istream& is, Random& rv )
   {
      std::cout << "Enter a random number seed "
           << "(between 1 and " << LONG_MAX - 1 << ", inclusive): " << std::endl;
...
// Empirical and Data-Driven Distributions
   ...
   int empiricalDiscrete( void )   // Empirical Discrete
   {
      ...
      static std::vector< float > f[ 2 ];   // f[ 0 ] is pdf and f[ 1 ] is cdf
      static float           max;
      static int              n;
      static bool             init = false;
      ...
      if ( !init ) {
         ...
         if ( !in ) {
            std::cerr << "Cannot open \"empiricalDiscrete\" input file"
                      << std::endl;
            exit( 1 );
         }
         int value;
   ...
   {                                     // distribution of 1-D data in a file
      static std::vector< float > v;         // vector for sampling with replacement
   ...
   {                                     // of multi-dimensional data
      ...
      static std::vector< float > v[ N_DIM ];
      static bool init = false;
      static int n;
   ...
   struct dSquared :
   public std::binary_function< cartesianCoord, cartesianCoord, bool > {
   ...
   {
      static std::vector< cartesianCoord > data;
      ...
      if ( !init ) {
         ...
         if ( !in ) {
            std::cerr << "Cannot open \"stochasticData\" input file"
                      << std::endl;
            exit( 1 );
         }
